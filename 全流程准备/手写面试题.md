...
console.log(result); // Output: "我叫xiaoming, 家住chengdu, 今年20岁了，明年就21岁了。"
...
```js
// 请在 30 分钟内实现简单的 html 源代码解析函数，返回所有节点树关系结构的数据
// ps: 不能使用浏览器 API 
function parse(html) {
  // 在这里完成这个函数的实现
}
...
// js 使用栈构建树的原理：一个变量向另一个变量复制引用类型的值（两个变量相互影响），即使那个变量已经 push 进一个数组里
```
